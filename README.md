...
- **全方位專業調光**：四通道獨立色調曲線（Composite／Red／Green／Blue）、Presence（紋理／清晰度／去朦朧）、三向色調分割（Color Grading）及相機風格渲染設定檔。
- **光學與幾何校正**：鏡頭變形、暗角、色差修正，水平／垂直透視校正與四角透視（Corner Pins）。
- **進階局部調整與離線 AI 遮罩**：線性與放射狀漸層、自由筆刷、亮度／色彩範圍遮罩、Apple Vision 離線前景／背景主體辨識，支援遮罩反轉、不透明度調節與去紅眼修復。
- **版本快照與專業預覽**：儲存多版本快照（支援單次 Compound Undo 還原與 A/B 比較）、高光／陰影溢出標記、目標色域超出警示，以及 sRGB／Display P3／Adobe RGB 軟體打樣。
- **可攜式 Sidecar v4**：中繼資料、評分、旗標、關鍵字、調光參數與快照全數持久化於可攜式 sidecar，並向前相容舊版格式。
- 支援單張及批次匯出，可設定 JPEG／PNG／HEIC／TIFF、品質、位元深度、尺寸、DPI、EXIF、中繼資料、檔名與檔案衝突處理。
- iPad 可同時管理多個來源；來源離線時仍保留索引與已快取縮圖，重新接上後可驗證身分並連回原來源。
- iPad 採自適應工作區，與 Mac 共享完整的專業檢視器目錄（Inspector Catalog），支援搜尋、收藏、釘選與區域重設。
- 介面完整支援英文、繁體中文、簡體中文、日文、韓文、德文、法文與西班牙文。
...
4. 調整曝光、四通道曲線、色彩分離、鏡頭校正、局部遮罩（含離線 AI 主體分割）或幾何透視，也可隨時切換原圖／編輯後比較模式。
5. 在右側檢視器切換至 **Snapshots** 分頁，隨時記錄不同調光版本的快照，並可進行單鍵還原（支援單筆 Compound Undo 撤銷還原）或與當前編輯狀態進行 A/B 比較；亦可開啟高光裁切（紅）、陰影死黑（藍）、色域警告（黃）與軟體打樣。
6. 使用 **Save Adjustments**（`⌘S`）儲存非破壞式編輯。原始 RAW 絕不會被改寫；來源資料夾將儲存可攜式 `.lumaharbor` sidecar v4。
7. 單張相片使用 **Export JPEG…**，多選相片使用 **Export Selected Photos…**。建議匯出到另一個資料夾，不要與來源 RAW 混放。
8. 圖庫可依評分、旗標、是否已編輯、關鍵字、相機、鏡頭、格式及拍攝日期篩選；需要批次處理時先進入選取模式。
...
5. Local 提供漸層、筆刷、範圍遮罩與裝置端離線 AI 前景辨識；幾何面板提供水平／垂直與四角透視校正。
6. Info 面板提供直方圖、相片中繼資料以及 **Snapshots 與專業預覽** 面板；頂部比較選單（Compare Menu）可直接選取任一快照進行 A/B 畫布對比。
7. Export 可設定 JPEG／PNG／HEIC／TIFF、品質、位元深度、尺寸、DPI、EXIF 與同名檔案處理，再儲存到「檔案」或「照片」。
8. 外接來源離線時保留索引與快取；重新接上後使用重新連接功能，App 會驗證來源身分，避免誤接到同名資料夾。
