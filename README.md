...
An iPad vertical slice (open a Sony RAW in place or copy it to the device, edit it with the same ten basic adjustments, non-destructive sidecar autosave) is in progress; the automated side (strict-concurrency build, the full XCTest suite, an iOS Simulator app build, and the Mac MVP acceptance gate) is exercised by [`Scripts/run-ipad-vertical-slice-acceptance.zsh`](Scripts/run-ipad-vertical-slice-acceptance.zsh), with the latest run recorded in [`docs/testing/reports/2026-08-24-ipad-raw-vertical-slice.md`](docs/testing/reports/2026-08-24-ipad-raw-vertical-slice.md). That report is not yet a sign-off: the automated run currently fails on a pre-existing, unrelated `RawFixtureTests` count check in `Scripts/run-mvp-acceptance.zsh`, and the five real-iPad hardware gates (in-place editing, "copy to this iPad", slider dragging, layout switching, and crash-recovery autosave) have not yet been run on real hardware.
