...
- 已知限制（非本輪新發現、不影響結論，但值得記錄）：
  1. 合成的 `⌘Z` 鍵盤事件無法穩定觸發 App 的 Undo（`System Events keystroke` 沒被 `UndoRedoKeyEquivalentFix` 的 local key-down monitor 接住），要用滑鼠點「編輯 › 復原」選單才能可靠觸發；程式碼裡的註解已記錄這是 SwiftUI `.undoRedo` CommandGroup 的既有限制。**2026-09-05 補測**：換用比 AppleScript keystroke 更接近硬體層級的 `CGEventCreateKeyboardEvent` 注入（先在 TextEdit 上驗證過這個注入方式本身確實有效），對 LumaHarbor 重試三次仍然一樣失敗，且確認過整個程式碼庫只有這一處 local monitor、沒有其他攔截者——排除了「只是 AppleScript/CUA 合成事件工具本身的缺陷」，提高了「這是會在真實實體鍵盤上重現的真實限制」的可能性，細節見上方 A11／A11 補測列。A11 因此不能被自動化結果支撐為 PASS，仍需真人用實體鍵盤直接按 `⌘Z` 補跑；若真人實測也失敗，建議另開一個工程調查任務用 Xcode 中斷點查 `NSEvent.addLocalMonitorForEvents` 的 handler 實際收到的事件內容。
...
- 下一步：真人在實機用實體鍵盤補跑 A11 的 `⌘Z`——2026-09-05 這輪已經把自動化能做的合成注入方式都試過（AppleScript keystroke、CUA、`CGEventCreateKeyboardEvent` 硬體等級注入），結論一致收斂在「local key-down monitor 沒收到／沒匹配到這個按鍵事件」，繼續在這個環境換更多種合成方式重試預期不會有新結果，真人實機測試是唯一能真正解答「這在實體鍵盤上到底過不過」的方法。若真人也重現失敗，建議直接開一個工程調查任務（Xcode 中斷點附加到 `UndoRedoKeyEquivalentFix.install` 的 handler，確認 `NSEvent.addLocalMonitorForEvents` 到底有沒有被呼叫、收到的 `charactersIgnoringModifiers`/`modifierFlags` 實際值），而不是回到手動 QA 清單裡繼續重試。完成 A11 後即可進入 Task 4.6（Phase 4 完整驗證輪：focused tests/`swift test`/`git diff --check`/隱私掃描彙整 + 這份清單的最終確認）。
