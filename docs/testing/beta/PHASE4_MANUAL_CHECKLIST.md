...
- **A11 最新狀態為 `PASS`**：2026-09-07 使用者先以實體鍵盤確認前一版按鍵沒有作用；Codex 追到 SwiftUI 每次 `menuNeedsUpdate` 都會重建並停用 `undo:`/`redo:` 項目，於 commit `853b837` 改成保留原 SwiftUI menu delegate、在其更新完成後再接到 `LumaHarborAppDelegate`。真 Release `.app` 的 CUA 端到端測試先確認 `⌘Z` 與 `⇧⌘Z` 都會改變曝光值與 Undo/Redo 狀態，之後使用者在同一修正版實際修改曝光並以實體鍵盤複測，回報復原與重做兩者均有作用；A11 因此正式 PASS。
...
| A11 | 對含有漸層的照片按 ⌘Z（Undo） | 最近一次漸層編輯（新增／拖曳／刪除／曝光調整）被復原，符合一般 undo 行為 | PASS | 舊版的合成事件與使用者實體鍵盤測試都曾失敗；選單／工具列 Undo 則可用。commit `853b837` 改用 menu delegate proxy，在 SwiftUI `menuNeedsUpdate` 完成後重新接線；真 Release `.app` 的 CUA 端到端測試確認 `⌘Z`／`⇧⌘Z` 可讓曝光 `+2.08 ↔ +1.08` 並同步切換 Undo/Redo 狀態。2026-09-07 使用者在此修正版實際修改曝光後，以實體鍵盤按 `⌘Z` 與 `⇧⌘Z`，回報復原與重做兩者均有作用；因此本項正式 PASS。舊版失敗與前期合成事件限制仍保留在「A11 補測」列，作為根因調查歷史。 |
...
- 結果：**PASS**。A 段（Task 4.3 線性漸層）A1-A13 與 B 段（Task 4.5 局部修護）B1-B13 全數通過；既有 sidecar JSON、截圖及匯出像素證據均保留。A11 的舊版實體鍵盤結果曾為 FAIL，但 commit `853b837` 完成第二次工程修復後，真 Release `.app` 的 CUA 端到端測試與使用者實體鍵盤 `⌘Z`／`⇧⌘Z` 複測均確認復原、重做有效。配合 focused tests（90 tests）、完整 `swift test`（1725 tests）、`git diff --check` 與隱私掃描全部通過，Phase 4 Task 4.6 完整驗證已完成。
...
- 已知限制（非本輪新發現、不影響結論，但值得記錄）：
  1. 舊版曾無法由 AppleScript、CUA、CGEvent 或使用者實體鍵盤穩定觸發 Undo；這些失敗用來定位 SwiftUI 重建 Edit 選單後覆蓋 target 的問題。commit `853b837` 的 menu delegate proxy 修正已由 CUA 與使用者實體鍵盤雙重確認有效，因此這不再是目前版本的已知功能限制；舊證據仍保留在 A11／A11 補測列作為歷史。
...
- 下一步：Phase 4 Task 4.6 已完成。後續驗收轉向 Phase 5／RC 剩餘項目：掛載 exFAT 測試磁碟後跑完整 MVP acceptance、六語母語審校與截斷檢查，以及 iPad hands-on checklist。
