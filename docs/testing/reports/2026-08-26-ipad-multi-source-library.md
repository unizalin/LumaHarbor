...
- `Scripts/run-ipad-vertical-slice-acceptance.zsh` is not present in this branch yet, so the library runner must not claim that vertical-slice gate as PASS.
...
2. Land or explicitly integrate the reviewed `Scripts/run-ipad-vertical-slice-acceptance.zsh`, then rerun the library runner.
