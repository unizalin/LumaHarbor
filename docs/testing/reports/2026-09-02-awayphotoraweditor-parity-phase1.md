...
| Privacy scan (`rg -n "/Users/\|/Volumes/\|/private/\|<TEAM_ID>\|teamIdentifier:\|DEVELOPMENT_TEAM"` over `git diff 8a400ed..HEAD`) | PASS | every hit is either (a) this exact `rg` pattern string quoted inside this phase's own `CURRENT.md` prose, (b) a redacted worktree placeholder, or (c) `/private/tmp/...DerivedData` build output paths from this round's own `xcodebuild` commands. Zero hits for the strict signing/device-only patterns (`<TEAM_ID>`, `teamIdentifier:`, `DEVELOPMENT_TEAM`) or for any real `/Volumes/` mount path — checked separately and confirmed empty. |
