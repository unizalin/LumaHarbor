...
2. **原圖比較（此前已測過通過）／Undo／Redo／整張 Reset All**——**通過**（Undo/Redo 走選單滑鼠點擊驗證，跟 Exposure/Contrast/Vibrance 多項疊加改動後 Reset All 一次歸零都確認正確；快捷鍵本身維持已知問題不變）。
3. **拉滑桿後立刻切換照片，先存檔才切走**——**通過**（這條本來就有自動化測試覆蓋，這次是人工雙重確認）。
4. **匯出流水號／取消清暫存檔**——**通過**（同一張照片匯出兩次到同資料夾，第二次正確加流水號、沒有覆蓋第一個檔案；匯出中途取消，Finder 檢查目的地資料夾乾淨、沒有殘留半成品或 `.tmp` 檔案）。
5. 錯誤情境下一步指引——offline／read-only／corrupt／disk-full 都已通過，unsupported 已確認這平台測不到，**這條視為完成，不用再測**。

**Gate F 人工功能矩陣（spec §Gate F 五項）全部完成。**
